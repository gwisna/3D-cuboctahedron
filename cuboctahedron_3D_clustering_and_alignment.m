...
close all;
%opening file
locs1 = h5read('0(1)_cuboctahedron_DNA-PAINT.hdf5', '/locs');%input file
    ...
    points_translate(:,1)=points(:,1)+x_shift_initial+x_step*(i-1);
        for j=1:1:-2*(y_shift_initial)/y_step 
            ...
            for k=1:1:360/n   %rotation alignment
                ...
                for l=1:size(index_weight,1)
                    for m=1:1:7
                       if index_weight(l)==m
                        D(l)=1*D(l);%weight factor is introduced
                      end
                    end
                end
...
if decoded(group_id,1)==0 && decoded(group_id,2)==1
